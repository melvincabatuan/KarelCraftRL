...
class VideoRecorder(Entity):
    ...
    def update(self):
        ...
        if self.recording:
            if self.i % self.frame_skip == 0:
                ...
                base.screenshot(
                    namePrefix='\\video_temp\\' + self.video_name +
                    '_' + str(self.i).zfill(4) + '.png',
                    defaultFilename=0,
                )
            self.i += 1
    ...
    def convert_to_gif(self):
        ...
        # try:
        #     imageio.mimsave(
        #         Path(f'{self.file_path.parent}/screenshots/{self.video_name}.gif'), images)
        # except Exception:
...
class VideoRecorderUI(WindowPanel):
    def __init__(self, **kwargs):
        ...
        self.start_button = Button(
            text='Start Recording [Shift+F12]', color=color.azure, on_click=self.start_recording)
